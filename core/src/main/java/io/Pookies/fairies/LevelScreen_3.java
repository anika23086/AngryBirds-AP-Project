...
public class LevelScreen_3 implements Screen, InputProcessor {
    private final Game game;
    private Stage stage;
    private SpriteBatch batch;
    private Texture levelBackground;
    private Bird pinkBird;
    ...
    private TriangleStructure triangleStructure1;
    ...
    private boolean stoneStructureDestroyed1 = false;
    private boolean stoneStructureDestroyed2 = false;
    private boolean triangleStructureDestroyed = false;
    ...
    private Rectangle stoneStructureRectangle1, stoneStructureRectangle2;
    private Rectangle triangleStructureRectangle;
    ...
    public LevelScreen_3(Game game) {
        ...
        pinkBird = new PinkBird(150, 240);
        purpleBird = new PurpleBird(150, 240);
        redBird = new RedBird(150, 240);
        unclePig1 = new UnclePig(920, 650);
        bossPig = new BossPig(1025, 375);
        woodStructure1 = new WoodStructure(900, 380);
        iceStructure1 = new IceStructure(1018, 150);
        iceStructure2 = new IceStructure(900, 150);
        stoneStructure1 = new StoneStructure(800, 400);
        stoneStructure2 = new StoneStructure(1100, 400);
        triangleStructure1 = new TriangleStructure(950, 500);
        ...
        stoneStructureRectangle1 = new Rectangle(800, 400, 100, 200);
        stoneStructureRectangle2 = new Rectangle(1100, 400, 100, 200);
        triangleStructureRectangle = new Rectangle(950, 500, 100, 200);
        ...
        birds = new Bird[]{pinkBird, purpleBird, redBird};
        currentBirdIndex = 0;
    ...
    private void setupErrorHandler() {
        if (!isErrorHandlerSet) {
            ...
            Gdx.app.error("LevelScreen_3", "Setting up error handler");
    ...
    public void show() {
        ...
        batch = new SpriteBatch();
        levelBackground = new Texture(Gdx.files.internal("level3_background.png"));
        ...
        Gdx.app.log("LevelScreen_3", "Pause button added to stage");
        Gdx.app.log("LevelScreen_3", "Pause button position: (" + pauseButton.getX() + ", " + pauseButton.getY() + "), size: (" + pauseButton.getWidth() + ", " + pauseButton.getHeight() + ")");
    }
    ...
    public void render(float delta) {
        try {
            ...
            if (allBirdsExhausted && !(pigDestroyed1 && pigDestroyed2)) {
                game.setScreen(new FailureScreen(game, 3));
                return;
            }
            ...
            if (gameStarted && !(pigDestroyed1 && pigDestroyed2)) {
                ...
                if (!pigDestroyed2) {
                    bossPig.update(delta);
                    pigRectangle2.setPosition(bossPig.getPosition().x, bossPig.getPosition().y);
                }
            }
            ...
            if (pigDestroyed1 && pigDestroyed2) {
                ...
                if (levelCompleteTimer >= LEVEL_COMPLETE_DELAY) {
                    LevelScreen.level3Completed = true;
                    game.setScreen(new SuccessScreen(game, 3));
                    return;
                }
            ...
            if (levelComplete) {
                levelCompleteTimer += delta;
                if (levelCompleteTimer >= LEVEL_COMPLETE_DELAY) {
                    if (!LevelScreen.level3Completed) {
                        LevelScreen.level3Completed = true;
                    }
                    game.setScreen(new SuccessScreen(game,3));
                    return;
            ...
            if (gameStarted && !(pigDestroyed1 && pigDestroyed2)) {
                ...
                if (unclePig1.getPosition().y < -100 || bossPig.getPosition().y < -100) {
                    pigDestroyed1 = true;
                    pigDestroyed2 = true;
                }
            }
            ...
            if (!stoneStructureDestroyed1) {
                stoneStructure1.render(batch);
            }
            if (!stoneStructureDestroyed2) {
                stoneStructure2.render(batch);
            }
            if (!triangleStructureDestroyed) {
                triangleStructure1.render(batch);
            }
    ...
    private void checkLevelCompletion() {
        if (pigDestroyed1 && pigDestroyed2 && !LevelScreen.level3Completed) {
            LevelScreen.level3Completed = true;
            game.setScreen(new SuccessScreen(game,3));
        }
    }
    ...
    private void handleCrash(Throwable throwable) {
        Gdx.app.error("LevelScreen_3", "Crash detected", throwable);
        try {
            Gdx.app.postRunnable(() -> {
                try {
                    ...
                    Gdx.app.error("LevelScreen_3", "Failed to show failure screen", e);
                    Gdx.app.exit();
                }
            });
        } catch (Exception e) {
            Gdx.app.error("LevelScreen_3", "Failed to handle crash", e);
            Gdx.app.exit();
        }
    }
    ...
    private void checkCollisions() {
        ...
        if (!stoneStructureDestroyed1 && birdRectangle.overlaps(stoneStructureRectangle1)) {
            handleStructureCollision(4);
        }
        if (!stoneStructureDestroyed2 && birdRectangle.overlaps(stoneStructureRectangle2)) {
            handleStructureCollision(5);
        }
        if (!triangleStructureDestroyed && birdRectangle.overlaps(triangleStructureRectangle)) {
            handleStructureCollision(6);
        }
    }
    ...
    private void handlePigCollision(int pigNumber) {
        if (pigNumber == 1 && !pigDestroyed1) {
            ...
            sparklePosition.set(bossPig.getPosition());
            sparkleTimer = SPARKLE_DURATION;
            sparkleAlpha = 1.0f;
    ...
    private void handleStructureCollision(int structureNumber) {
        switch (structureNumber) {
            ...
            case 3:
                ...
                break;
            case 4:
                stoneStructureDestroyed1 = true;
                break;
            case 5:
                stoneStructureDestroyed2 = true;
                break;
            case 6:
                triangleStructureDestroyed = true;
                break;
        }
    ...
    private void checkPigsOnDestroyedStructures() {
        ...
        if (stoneStructureDestroyed1) {
            Rectangle structureRect4 = new Rectangle(800, 400, 100, 200);

            if (!pigDestroyed1 && isPigDirectlyAboveStructure(structureRect4)) {
                handlePigFall(1);
            }
            if (!pigDestroyed2 && isPigDirectlyAboveStructure(structureRect4)) {
                handlePigFall(2);
            }
        }

        if (stoneStructureDestroyed2) {
            Rectangle structureRect5 = new Rectangle(1100, 400, 100, 200);

            if (!pigDestroyed1 && isPigDirectlyAboveStructure(structureRect5)) {
                handlePigFall(1);
            }
            if (!pigDestroyed2 && isPigDirectlyAboveStructure(structureRect5)) {
                handlePigFall(2);
            }
        }

        if (triangleStructureDestroyed) {
            Rectangle structureRect6 = new Rectangle(950, 500, 100, 200);

            if (!pigDestroyed1 && isPigDirectlyAboveStructure(structureRect6)) {
                handlePigFall(1);
            }
            if (!pigDestroyed2 && isPigDirectlyAboveStructure(structureRect6)) {
                handlePigFall(2);
            }
        }
    }
    ...
    public void dispose() {
        try {
            ...
            if (triangleStructure1 != null) {
                triangleStructure1.dispose();
                triangleStructure1 = null;
            }
            ...
            Gdx.app.error("LevelScreen_3", "Error during disposal", e);
        }
    }
}
